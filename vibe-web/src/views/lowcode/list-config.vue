...
/**
 * 列表配置管理（spec 阶段三 - Task A4.2）
 *
 * 功能：分页查询 + 新增/编辑（SchemaDesigner list 模式）+ 复制 + 导出/导入 JSON + 删除。
 */
...
type ListConfigEditDTO = LowcodeListConfigDTO & { id?: number }

const loading = ref(false)
const dataSource = ref<LowcodeListConfigVO[]>([])
...
async function loadData() {
  loading.value = true
  try {
    const res = (await pageListConfigs({
      ...query,
    ...
    console.error('[list-config] load failed:', e)
  } finally {
    loading.value = false
  }
}
...
const formData = reactive<ListConfigEditDTO>({
  configCode: '',
  configName: '',
  schemaJson: '',
  status: 1,
...
function openCreate() {
  ...
  Object.assign(formData, {
    ...
    status: 1,
    description: ''
  } as ListConfigEditDTO)
  designerVisible.value = true
}

function openEdit(row: LowcodeListConfigVO) {
  isEdit.value = true
  Object.assign(formData, {
    id: row.id,
...
async function handleSubmit() {
  ...
  if (!formData.schemaJson) {
    message.warning('请使用设计器设计列表 Schema')
    return
  }
  ...
  try {
    ...
    if (isEdit.value && formData.id) {
      await updateListConfig(formData.id, dto)
    ...
    console.error('[list-config] submit failed:', e)
  } finally {
    designerLoading.value = false
  }
}

async function handleCopy(row: LowcodeListConfigVO) {
  try {
    await copyListConfig(row.id)
    message.success('复制成功')
    loadData()
  } catch (e) {
    console.error('[list-config] copy failed:', e)
  }
}

function handleDelete(row: LowcodeListConfigVO) {
  Modal.confirm({
    title: '确认删除',
    ...
    onOk: async () => {
      try {
        await deleteListConfig(row.id)
        message.success('删除成功')
        loadData()
      } catch (e) {
        console.error('[list-config] delete failed:', e)
      }
    }
  })
}

async function handleExport(row: LowcodeListConfigVO) {
  try {
    const blob = await exportListConfigJson(row.id)
    downloadBlob(blob, `${row.configCode}.json`)
    message.success('导出成功')
  } catch (e) {
    console.error('[list-config] export failed:', e)
  }
}
...
async function handleImport(payload: { schemaJson: string; templateId?: number }) {
  try {
    const newConfig: LowcodeListConfigDTO = {
      configCode: `import_${Date.now()}`,
      configName: `导入列表_${new Date().toLocaleString()}`,
      schemaJson: payload.schemaJson,
      status: 1,
      description: '通过 JSON 导入'
    }
    await importListConfig(newConfig)
    message.success('导入成功')
    loadData()
  } catch (e) {
    console.error('[list-config] import failed:', e)
  }
}
...
<template>
  <PageContainer title="列表配置" description="低代码列表 Schema 设计与管理">
    <template #extra>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space size="small" wrap>
              <a @click="openEdit(record as LowcodeListConfigVO)"><EditOutlined /> 编辑</a>
              <a-divider type="vertical" />
              <a @click="handleCopy(record as LowcodeListConfigVO)"><CopyOutlined /> 复制</a>
              <a-divider type="vertical" />
              <a @click="handleExport(record as LowcodeListConfigVO)"><DownloadOutlined /> 导出</a>
              <a-divider type="vertical" />
              <a class="danger-link" @click="handleDelete(record as LowcodeListConfigVO)"><DeleteOutlined /> 删除</a>
            </a-space>
          </template>
        </template>
    ...
    >
        ...
        <a-row :gutter="16">
          <a-col :span="8">
            <a-form-item label="配置编码" required>
              <a-input v-model:value="formData.configCode" placeholder="如 customer_list" :disabled="isEdit" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="配置名称" required>
              <a-input v-model:value="formData.configName" placeholder="如 客户列表" />
            </a-form-item>
          </a-col>
          <a-col :span="4">
            <a-form-item label="状态">
      ...
      <div class="designer-wrapper">
        <SchemaDesigner v-model="formData.schemaJson" mode="list" />
      </div>
