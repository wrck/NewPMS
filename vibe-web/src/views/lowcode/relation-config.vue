...
/**
 * 关联页配置管理（spec 阶段三 - Task A4.4）
 *
 * 功能：分页查询 + 新增/编辑（SchemaDesigner relation 模式）+ 复制 + 导出/导入 JSON + 删除。
 */
...
import {
  ...
  copyRelationConfig,
  exportRelationConfigJson,
  importRelationConfig,
  downloadBlob
} from '@/api/lowcode'
import type {
...
type RelationConfigEditDTO = LowcodeRelationConfigDTO & { id?: number }

const loading = ref(false)
const dataSource = ref<LowcodeRelationConfigVO[]>([])
...
async function loadData() {
  loading.value = true
  try {
    const res = (await pageRelationConfigs({
      ...query,
    ...
    console.error('[relation-config] load failed:', e)
  } finally {
    loading.value = false
  }
}
...
const formData = reactive<RelationConfigEditDTO>({
  configCode: '',
  configName: '',
  schemaJson: '',
  status: 1,
...
function openCreate() {
  ...
  Object.assign(formData, {
    ...
    status: 1,
    description: ''
  } as RelationConfigEditDTO)
  designerVisible.value = true
}

function openEdit(row: LowcodeRelationConfigVO) {
  isEdit.value = true
  Object.assign(formData, {
    id: row.id,
...
async function handleSubmit() {
  ...
  if (!formData.schemaJson) {
    message.warning('请使用设计器设计关联页 Schema')
    return
  }
  ...
  try {
    ...
    if (isEdit.value && formData.id) {
      await updateRelationConfig(formData.id, dto)
    ...
    console.error('[relation-config] submit failed:', e)
  } finally {
    designerLoading.value = false
  }
}

async function handleCopy(row: LowcodeRelationConfigVO) {
  try {
    await copyRelationConfig(row.id)
    message.success('复制成功')
    loadData()
  } catch (e) {
    console.error('[relation-config] copy failed:', e)
  }
}

function handleDelete(row: LowcodeRelationConfigVO) {
  Modal.confirm({
    title: '确认删除',
    ...
    onOk: async () => {
      try {
        await deleteRelationConfig(row.id)
        message.success('删除成功')
        loadData()
      } catch (e) {
        console.error('[relation-config] delete failed:', e)
      }
    }
  })
}

async function handleExport(row: LowcodeRelationConfigVO) {
  try {
    const blob = await exportRelationConfigJson(row.id)
    downloadBlob(blob, `${row.configCode}.json`)
    message.success('导出成功')
  } catch (e) {
    console.error('[relation-config] export failed:', e)
  }
}
...
async function handleImport(payload: { schemaJson: string; templateId?: number }) {
  try {
    const newConfig: LowcodeRelationConfigDTO = {
      configCode: `import_${Date.now()}`,
      configName: `导入关联页_${new Date().toLocaleString()}`,
      schemaJson: payload.schemaJson,
      status: 1,
      description: '通过 JSON 导入'
    }
    await importRelationConfig(newConfig)
    message.success('导入成功')
    loadData()
  } catch (e) {
    console.error('[relation-config] import failed:', e)
  }
}
...
<template>
  <PageContainer title="关联页配置" description="低代码主从关联页 Schema 设计与管理">
    <template #extra>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space size="small" wrap>
              <a @click="openEdit(record as LowcodeRelationConfigVO)"><EditOutlined /> 编辑</a>
              <a-divider type="vertical" />
              <a @click="handleCopy(record as LowcodeRelationConfigVO)"><CopyOutlined /> 复制</a>
              <a-divider type="vertical" />
              <a @click="handleExport(record as LowcodeRelationConfigVO)"><DownloadOutlined /> 导出</a>
              <a-divider type="vertical" />
              <a class="danger-link" @click="handleDelete(record as LowcodeRelationConfigVO)"><DeleteOutlined /> 删除</a>
            </a-space>
          </template>
        </template>
    ...
    >
        ...
        <a-row :gutter="16">
          <a-col :span="8">
            <a-form-item label="配置编码" required>
              <a-input v-model:value="formData.configCode" placeholder="如 customer_relation" :disabled="isEdit" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="配置名称" required>
              <a-input v-model:value="formData.configName" placeholder="如 客户关联页" />
            </a-form-item>
          </a-col>
          <a-col :span="4">
            <a-form-item label="状态">
      ...
      <div class="designer-wrapper">
        <SchemaDesigner v-model="formData.schemaJson" mode="relation" />
      </div>
