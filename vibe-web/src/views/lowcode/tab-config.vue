...
/**
 * 标签页配置管理（spec 阶段三 - Task A4.3）
 *
 * 功能：分页查询 + 新增/编辑（SchemaDesigner tab 模式）+ 复制 + 导出/导入 JSON + 删除。
 */
...
type TabConfigEditDTO = LowcodeTabConfigDTO & { id?: number }

const loading = ref(false)
const dataSource = ref<LowcodeTabConfigVO[]>([])
...
async function loadData() {
  loading.value = true
  try {
    const res = (await pageTabConfigs({
      ...query,
    ...
    console.error('[tab-config] load failed:', e)
  } finally {
    loading.value = false
  }
}
...
const formData = reactive<TabConfigEditDTO>({
  configCode: '',
  configName: '',
  schemaJson: '',
  status: 1,
...
function openEdit(row: LowcodeTabConfigVO) {
  isEdit.value = true
  Object.assign(formData, {
    id: row.id,
...
async function handleSubmit() {
  ...
  if (!formData.schemaJson) {
    message.warning('请使用设计器设计标签页 Schema')
    return
  }
  ...
  try {
    ...
    if (isEdit.value && formData.id) {
      await updateTabConfig(formData.id, dto)
    ...
    console.error('[tab-config] submit failed:', e)
  } finally {
    designerLoading.value = false
  }
}

async function handleCopy(row: LowcodeTabConfigVO) {
  try {
    await copyTabConfig(row.id)
    message.success('复制成功')
    loadData()
  } catch (e) {
    console.error('[tab-config] copy failed:', e)
  }
}

function handleDelete(row: LowcodeTabConfigVO) {
  Modal.confirm({
    title: '确认删除',
    ...
    onOk: async () => {
      try {
        await deleteTabConfig(row.id)
        message.success('删除成功')
        loadData()
      } catch (e) {
        console.error('[tab-config] delete failed:', e)
      }
    }
  })
}

async function handleExport(row: LowcodeTabConfigVO) {
  try {
    const blob = await exportTabConfigJson(row.id)
    downloadBlob(blob, `${row.configCode}.json`)
    message.success('导出成功')
  } catch (e) {
    console.error('[tab-config] export failed:', e)
  }
}
...
async function handleImport(payload: { schemaJson: string; templateId?: number }) {
  try {
    const newConfig: LowcodeTabConfigDTO = {
      configCode: `import_${Date.now()}`,
      configName: `导入标签页_${new Date().toLocaleString()}`,
      schemaJson: payload.schemaJson,
      status: 1,
      description: '通过 JSON 导入'
    }
    await importTabConfig(newConfig)
    message.success('导入成功')
    loadData()
  } catch (e) {
    console.error('[tab-config] import failed:', e)
  }
}
...
<template>
  <PageContainer title="标签页配置" description="低代码标签页 Schema 设计与管理">
    <template #extra>
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <a-space size="small" wrap>
              <a @click="openEdit(record as LowcodeTabConfigVO)"><EditOutlined /> 编辑</a>
              <a-divider type="vertical" />
              <a @click="handleCopy(record as LowcodeTabConfigVO)"><CopyOutlined /> 复制</a>
              <a-divider type="vertical" />
              <a @click="handleExport(record as LowcodeTabConfigVO)"><DownloadOutlined /> 导出</a>
              <a-divider type="vertical" />
              <a class="danger-link" @click="handleDelete(record as LowcodeTabConfigVO)"><DeleteOutlined /> 删除</a>
            </a-space>
          </template>
        </template>
    ...
    >
        ...
        <a-row :gutter="16">
          <a-col :span="8">
            <a-form-item label="配置编码" required>
              <a-input v-model:value="formData.configCode" placeholder="如 project_tab" :disabled="isEdit" />
            </a-form-item>
          </a-col>
          <a-col :span="8">
            <a-form-item label="配置名称" required>
              <a-input v-model:value="formData.configName" placeholder="如 项目标签页" />
            </a-form-item>
          </a-col>
          <a-col :span="4">
            <a-form-item label="状态">
      ...
      <div class="designer-wrapper">
        <SchemaDesigner v-model="formData.schemaJson" mode="tab" />
      </div>
